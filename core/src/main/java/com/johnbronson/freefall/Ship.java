...
public class Ship {
    ...
    public void update(float deltaTime) {
        if (solidBoostTimer > 0) {
            ...
            double angleRad = angle * Constants.MILS_TO_RADIANS;
            vx += (float)Math.cos(angleRad) * Constants.SOLID_BOOST_ACCELERATION * deltaTime;
            vy += (float)Math.sin(angleRad) * Constants.SOLID_BOOST_ACCELERATION * deltaTime;
        }
    ...
    public float getSolidBoostTimer() {
        return solidBoostTimer;
    }
